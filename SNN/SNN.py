...
# Curvas de entrenamiento (análogas a las de SNN_reg+clas.py, sin Kendall)
# El Trainer guarda: loss_hist["train"][e] = lista de pérdidas por batch de la
# época e (0-based), y loss_hist/acc_hist["validation"][e] = valor tras la
# época e (la clave 0 es la validación previa al entrenamiento).
...
fig_tc.tight_layout()
fig_tc.savefig("training_curves_SNN_simple_50_epocas.png", dpi=300, bbox_inches="tight")
plt.close(fig_tc)

cm_metric = train_Epos_spk.ConfusionMatrix(num_classes=nClasses)
cm = cm_metric.compute().cpu().numpy()
np.savetxt("confusion_matrix_spiking.txt", cm, fmt="%d")
...
fig_cm.tight_layout()
fig_cm.savefig("confusion_matrix_SNN_simple_50_epocas.png", dpi=300, bbox_inches="tight")
...
fig.tight_layout()
fig.savefig("probabilidades_SNN_simple_50_epocas.png", dpi=300, bbox_inches="tight")
print("terminado")
plt.close(fig)
