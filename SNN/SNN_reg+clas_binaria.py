...
# Version binaria de SNN_reg+clas_nocls.py: sin SNN clasificadora intermedia
# (la cabecera recibe el spiketrain de la matriz de fotones + las variables de
# regresion) y clasificando solo el par de particulas de PAIR.
...
# Clasificacion binaria: par de clases originales del dataset a enfrentar
# (0: proton, 1: kaon, 2: pion). Se reetiquetan como 0/1; el resto se filtra.
PAIR = (0, 1)
...
def spikegen_multi(data, multiplicity=4):
    ...
    spike_data = torch.zeros(
        og_shape[1], og_shape[0], multiplicity * og_shape[2],
        device=data.device,
    )
...
class SpikingClassificationHead(nn.Module):
    ...
    def __init__(self, layers, neuron_params, out_population=1):
        ...
        self.neurons = nn.ModuleList(
            snn.Leaky(**neuron_params) for _ in range(len(layers) - 1)
        )
...
class Hybrid_Net(nn.Module):
    def __init__(
        ...
        self.head_cls = SpikingClassificationHead(
            layers=[head_in, 64, 32, multiplicity * n_classes],
    ...
    def forward(self, data):
        ...
        # spiketrain de la matriz de fotones: (timesteps, batch, 400)
...
class BinaryDataset(torch.utils.data.Dataset):
    def __init__(self, base, class_a, class_b):
        ...
        self.indices = [i for i in range(len(base))
                        if int(base[i][1][0]) in self.mapping]

    def __len__(self):
        return len(self.indices)

    def __getitem__(self, i):
        data, target = self.base[self.indices[i]]
        target = list(target)
        target[0] = self.mapping[int(target[0])]
        return data, tuple(target)


dataset_base = ds.build_dataset(
...
fig.tight_layout()
save_figure(fig, "training_curves_reg+clas_nocls_binaria_proton_kaon_50_epocas.png", dpi=300, bbox_inches="tight")
plt.close(fig)
...
for t, p in zip(all_targets, all_preds):
    cm[t, p] += 1
np.savetxt("confusion_matrix_reg+clas_nocls_binaria.txt", cm, fmt="%d")
...
fig_cm.tight_layout()
save_figure(fig_cm, "confusion_matrix_SNN_reg+clas_nocls_binaria_proton_kaon_50_epocas.png", dpi=300, bbox_inches="tight")
plt.close(fig_cm)
...
fig.tight_layout()
save_figure(fig, "probabilidades_reg+clas_nocls_binaria_proton_kaon_50_epocas.png", dpi=300, bbox_inches="tight")
plt.close(fig)
...
save_figure(fig_roc, "roc_reg+clas_nocls_binaria_proton_kaon_50_epocas.png", dpi=300, bbox_inches="tight")
plt.close(fig_roc)
...
fig.tight_layout()
save_figure(fig, "residuos_regresion_reg+clas_nocls_binaria_proton_kaon_50_epocas.png", dpi=300, bbox_inches="tight")
plt.close(fig)
